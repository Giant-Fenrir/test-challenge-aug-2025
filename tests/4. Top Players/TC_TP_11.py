...
def browser():
    ...
    test_top_players_all_filters(driver)
    driver.quit()


def test_top_players_all_filters(driver):
    """
    ID: TC-TP-11
    Title: List all top players using all filters
    Description: Verify if a list of top players will be shown given a position, nationality and league
    Steps:  1. Go to Top Players
            2. Click Clear filters
            3. Select any top x
            4. Select a postion, nationality and league
            5. Click Search
    Expected result: A list of top X players from the selected filters should be displayed
    """
    status = True
    result = ""
    ...
    Select(driver.find_element(By.XPATH, "//*[@id='root']/div/div/div[1]/div/div[1]/div[3]/select")).select_by_value("Brazil")
    Select(driver.find_element(By.XPATH, "//*[@id='root']/div/div/div[1]/div/div[1]/div[4]/select")).select_by_value("ST")
    ...
    test_result(status, result, "Top Player", "TC-TP-11")
