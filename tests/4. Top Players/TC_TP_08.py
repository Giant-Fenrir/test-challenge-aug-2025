...
def browser():
    ...
    test_top_players_by_position(driver)
    driver.quit()


def test_top_players_by_position(driver):
    """
    ID: TC-TP-08
    Title: List all top players given a position
    Description: Verify if a list of top players will be shown given a position
    Steps:  1. Go to Top Players
            2. Click Clear filters
            3. Select any top x
            4. Select a position
            5. Click Search
    Expected result: A list of top X players from the selected position should be displayed
    """
    status = True
    result = ""
    ...
    test_result(status, result, "Top Player", "TC-TP-08")
