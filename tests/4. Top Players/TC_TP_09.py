...
def browser():
    ...
    test_top_players_by_nationality(driver)
    driver.quit()


def test_top_players_by_nationality(driver):
    """
    ID: TC-TP-09
    Title: List all top players given a nationality
    Description: Verify if a list of top players will be shown given a nationality
    Steps:  1. Go to Top Players
            2. Click Clear filters
            3. Select any top x
            4. Select a nationality
            5. Click Search
    Expected result: A list of top X players from the selected nationality should be displayed
    """
    status = True
    result = ""
    ...
    test_result(status, result, "Top Player", "TC-TP-09")
