...
def browser():
    ...
    test_best_team_all_filters(driver)
    driver.quit()


def test_best_team_all_filters(driver):
    """
    ID: TC-TT-04
    Title: List the best players from a league and nationality to form a team
    Description: Verify if, given a league and nationality, the best players to form a team will be shown
    Steps:  1. Go to Top Team
            2. Click Clear Filters
            3. Select a league and nationality
            4. Click Search
    Expected result: A top team with all filters should be displayed
    """
    status = True
    result = ""
    ...
    try:
        driver.find_element(By.XPATH, ("//span[@class='title' and text()='Ryder Matos']"))
        result += "[SUCCESS]:Filter applied"
    except:
        result += "[FAIL]:Filter not applied"
        status = False

    test_result(status, result, "Top Team", "TC-TT-04")
